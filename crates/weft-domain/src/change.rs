...
impl UnixMillis {
    /// Creates a timestamp measured in milliseconds since the Unix epoch.
    ///
    /// # Errors
    ///
    /// Returns [`ChangeError::InvalidTimestamp`] for values before the epoch.
    pub const fn new(value: i64) -> Result<Self, ChangeError> {
        if value < 0 {
...
impl Display for ChangeError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidTimestamp => formatter.write_str("timestamp cannot be before Unix epoch"),
...
mod tests {
    ...
    fn revision(id: &str) -> NewRevision {
        NewRevision::new(
            ...
            UnixMillis::new(1_777_777_777_000).unwrap(),
    ...
    fn creates_a_linear_revision_sequence() {
        ...
        assert_eq!(first.created_by().as_str(), "agent-1");
        assert_eq!(first.created_at().value(), 1_777_777_777_000);
    ...
    fn rejects_invalid_base_and_artifact_identity() {
        ...
        assert!(matches!(
            UnixMillis::new(-1),
            Err(ChangeError::InvalidTimestamp)
        ));
    }
}
