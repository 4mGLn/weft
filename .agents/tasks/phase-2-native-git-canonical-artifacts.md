...
- **Out of scope:** Public CLI and Windows materialization.
...
6. A running attempt is reconciled from the observed target; only a confirmed result creates a durable receipt, while divergence remains unresolved.
...
6. Reconcile a running attempt — proof: provider test records confirmed target evidence and then persists a receipt and succeeded state.

## Validation record

| Check | Command/test | Result | Evidence |
| --- | --- | --- | --- |
| Focused | `cargo test -p weft-native-git` | Passed | Discovery, capture, reconstruction, overlap, composition, target-CAS, durable execution, and reconciliation tests. |
| Domain/contract | `cargo test --workspace --all-targets` | Passed | 37 domain tests plus 6 Native Git tests. |
| Concurrency/recovery | Native Git persisted-attempt test | Passed | A running attempt is reconciled from an observed target to a durable receipt and succeeded state. |
| Provider integration | Native Git temporary repository tests | Passed | Materialized staged tree ID equals exact target tree ID; composition and target CAS are guarded; durable outcomes reconcile explicitly. |
...
- **Residual risks:** Process deadlines/output caps, non-UTF-8 paths, submodules, crash injection around the provider CAS, and divergent-conflict resolution workflow remain unimplemented.
- **Unavailable evidence:** Windows file-mode/symlink behavior and crash injection during real provider mutations.
- **Follow-up, owner, resumption condition:** Primary agent adds crash injection around provider CAS and exercises divergent conflict/reconciliation paths; each provider mutation needs failure evidence.
