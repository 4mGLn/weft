...
The first runtime release is a relocatable archive built and smoke-tested on Ubuntu 24.04 x86_64, containing the `weft` binary, install/uninstall helpers, a small operator reference (`README.md`, `GETTING_STARTED.md`, `MANUAL.md`, `USAGE.md`, and `LICENSE`), the embedded `SBOM.cdx.json`, and the archive manifest. The repository documentation tree, development scripts, and source contracts remain in the repository and are not runtime payload. Other distributions and glibc versions are not supported claims. It is a local CLI, not a daemon or hosted service. The state directory is caller-selected and retained across upgrade, rollback, and uninstall. Native Git requires Git 2.38 or newer. GitButler remains an optional, explicitly capability-gated development adapter requiring exactly `but 0.22.0`.

Tags matching semantic version `vMAJOR.MINOR.PATCH` build from the tagged source, run the full gate, smoke-test installation from the archive, emit CI-only SHA-256 sidecars and a deterministic CycloneDX inventory, and publish only the runtime archive through GitHub Releases. The SBOM remains embedded in that archive; GitHub artifact attestations cover that archive. Artifact signing, vulnerability scanning, additional operating systems/architectures, package-manager distribution, auto-update, services, and hosted deployment remain unsupported until separately evidenced.
