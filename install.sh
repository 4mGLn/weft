#!/bin/sh
# Download a verified supported runtime release, then invoke its archive installer.
set -eu
...
case "$(uname -s)" in
    ...
    Darwin) case "$(uname -m)" in arm64) target=aarch64-apple-darwin ;; x86_64) target=x86_64-apple-darwin ;; *) echo "unsupported macOS architecture" >&2; exit 1 ;; esac ;;
...
' "$target")
url=$(printf '%s\n' "$asset" | sed -n '1p'); expected=$(printf '%s\n' "$asset" | sed -n '2p')
test -n "$url" && test -n "$expected"
root=$(mktemp -d); trap 'rm -rf "$root"' EXIT HUP INT TERM
archive="$root/runtime.tar.gz"; curl --proto '=https' --tlsv1.2 -fsSL "$url" -o "$archive"
...
tar -xzf "$archive" -C "$root"; package=$(find "$root" -mindepth 1 -maxdepth 1 -type d -name 'weft-*' -print)
test "$(printf '%s\n' "$package" | wc -l)" -eq 1; PREFIX="$prefix" "$package/install.sh"
